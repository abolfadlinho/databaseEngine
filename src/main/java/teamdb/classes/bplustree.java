...
public class bplustree implements Serializable{
	...
	private int binarySearch(DictionaryPair[] dps, int numPairs, Comparable t) {
		...
		return Arrays.binarySearch(dps, 0, numPairs, new DictionaryPair(t, new Vector<String>()), c);
	}
	...
	public void edit(Comparable key,String previousPage,String newPage) {
		if (isEmpty()) {
			...
			if (dpIndex < 0) {
				...
				// Successfully delete the dictionary pair
				ln.edit(dpIndex,previousPage,newPage);
			}
		}
	}
	...
	public LeafNode insert(Comparable key, String value){
		if (isEmpty()) {
			...
			LeafNode ln = new LeafNode(this.m, new DictionaryPair(key, vector));
			...
			Vector<String> vector = new Vector<String>();
			vector.add(value);
			DictionaryPair pair = new DictionaryPair(key, vector);

			LeafNode ret=ln; // Used to return the leaf node containing inserted key

			// Insert into leaf node fails if node becomes overfull
			if (!ln.insert(pair)) {
				
				// Sort all the dictionary pairs with the included pair to be inserted
				ln.dictionary[ln.numPairs] = pair;
				ln.numPairs++;
				sortDictionary(ln.dictionary);
				...
				// Checking if new node contains key rather than the previous leaf
				for(int i=0;i<newLeafNode.dictionary.length;i++){
					if(newLeafNode.dictionary[i]==null) break;
			
					if(newLeafNode.dictionary[i].key==key){
						ret = newLeafNode;
						break;
					}
				}
	 ...
	 */
	public Hashtable<Comparable,Vector<String>> search(Comparable lowerBound, Comparable upperBound,boolean lowerBoundInclusive,boolean upperBoundInclusive) {

		// Instantiate Double array to hold values
		Hashtable<Comparable,Vector<String>> values = new Hashtable<Comparable,Vector<String>>();
		...
		while (currNode != null) {
			...
			for (DictionaryPair dp : dps) {
				...
				if ((lowerBound != null && upperBound != null && (lowerBoundInclusive ? lowerBound.compareTo(dp.key) <= 0 : lowerBound.compareTo(dp.key) < 0) && (upperBoundInclusive ? dp.key.compareTo(upperBound) <= 0 : dp.key.compareTo(upperBound) < 0)) ||
					(lowerBound == null && upperBound != null && (upperBoundInclusive ? dp.key.compareTo(upperBound) <= 0 : dp.key.compareTo(upperBound) < 0)) ||
					(lowerBound != null && upperBound == null && (lowerBoundInclusive ? lowerBound.compareTo(dp.key) <= 0 : lowerBound.compareTo(dp.key) < 0)) ||
					(lowerBound == null && upperBound == null)) {

					if (!values.containsKey(dp.key)) {
						// Unique key to be added
						Vector<String> v = new Vector<>();
						v.addAll(dp.value); // adds vector (value) to vector (v)
						values.put(dp.key, v);
					} else {
						// key already exists in Hashtable
						// add vector(value) to already added vector
						values.get(dp.key).addAll(dp.value);
					}
				}
			}
	...
	public class LeafNode extends Node{
		...
		public void delete(int index,String pageName) {

			// Delete dictionary pair from leaf
			DictionaryPair pair = this.dictionary[index];
			Vector<String> pageNames = pair.value;
			pageNames.removeElement(pageName);
			if(pageNames.size()==0){
				this.dictionary[index] = null;
				// That page is the only one
				numPairs--;
			}
		}

		public void edit(int index,String pageName) {
			DictionaryPair pair = this.dictionary[index];
			Vector<String> pageNames = pair.value;
			pageNames.remove(0); // Because this is only done when we are inserting with an empty text
			pageNames.add(pageName);
		}

		public void edit(int index,String previousPage,String newPage) {
			DictionaryPair pair = this.dictionary[index];
			Vector<String> pageNames = pair.value;
			pageNames.removeElement(previousPage); // Because this is only done when we are inserting with an empty text
		 ...
		 */
		public boolean insertOld(DictionaryPair dp) {
			if (this.isFull()) {
				...
				int index=-1;
				int i = 0;
				while(i<this.dictionary.length && dictionary[i]!=null){
					// While loop Because dictionary.length always returns its total length m not used length
					if(dictionary[i].key==dp.key){
						index=i;
						break;
					}
					i++;
				...
				if(index<0){
					// Key is not already in dictionary keys
					// So just add a new dictionary pair
					this.dictionary[numPairs] = dp;
					numPairs++;
				}else{
					// Key is already in dictionary keys
					// Add the string to the value (vector) of pair
					this.dictionary[index].value.add(dp.value.get(0));
				}
	...
	public class DictionaryPair implements Comparable<DictionaryPair>, Serializable {
		...
		public String toString(){
			String res="Key: " + key + " Value: " + value;
			return res;
		}
	}

	public static String helperforindexsearch(bplustree bp, Comparable comp){
		LeafNode ln = bp.insert(comp, "");
		int index=-100;
		for(int i=0;i<ln.dictionary.length;i++){
			if(ln.dictionary[i]==null) break;
		
			Comparable compp =  ln.dictionary[i].key;
		
			if(comp.compareTo(compp) == 0){     // not sure if that is the correct way to compare the comparables
		...
		if(index==ln.dictionary.length-1 || ln.dictionary[index+1]==null){
		
		   LeafNode rightSibling = ln.rightSibling;
			index = -1;
			ln = rightSibling;
		}
	   
	   if (ln == null){  //if rightsibling = null
		
		bp.delete(comp);
		return "we are going to insert our tuple in the last page if not full handled in DB APP";
	   }
	
		String page = ln.dictionary[index+1].value.get(0);
	
		bp.delete(comp); //we added the key and empty vector of page file names just to get the next page's name (so we'll remove it)
		return page; //page name bta3et el tuple eli ha insert ablo (same page name of the tuple to insert)
